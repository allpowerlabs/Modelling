...
triax.plot(data.frame(CHO$C,CHO$H,CHO$O),show.grid=TRUE,col.grid="#EEEEEE",show.legend=FALSE,point.labels=CHO$name,pch=15,col.symbols=CHO$col)
@
\\
\newpage
<<fig=TRUE ,echo=FALSE>>=
print(bwplot(data$Class~data$HHV_meas,xlab="HHV [MJ/kg]"))
@
\\
\newpage
<<fig=TRUE ,echo=FALSE>>=
print(bwplot(data$Class~data$Ash,xlab="Ash [%]"))
@
\\
\newpage
<<fig=TRUE ,echo=FALSE>>=
print(bwplot(data$Class~data$Cl,xlab="Chlorine [%]"))
@
\\
\newpage
<<results=tex>>=
